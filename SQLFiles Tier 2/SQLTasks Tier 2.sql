...
SELECT *
FROM Facilities
WHERE `Facilities`.`facid` BETWEEN 1 AND 5
...
SELECT DISTINCT Facilities.name, CONCAT_WS(', ',Members.surname, Members.firstname) AS user
FROM ((Bookings
...
SELECT starttime, name, CONCAT_WS(', ',Members.surname, Members.firstname) AS user, 
(CASE WHEN Facilities.membercost > 30 THEN Facilities.membercost
WHEN Facilities.guestcost > 30 THEN Facilities.guestcost
END) AS cost
FROM ((Bookings
INNER JOIN Facilities ON Bookings.facid = Facilities.facid
...
SELECT bookid, (SELECT name FROM Facilities WHERE Bookings.facid = Facilities.facid) AS name, (SELECT CONCAT_WS(', ',Members.surname, Members.firstname) FROM Members WHERE Bookings.memid = Members.memid) AS user, (SELECT CASE WHEN Facilities.membercost > 30 THEN Facilities.membercost
WHEN Facilities.guestcost > 30 THEN Facilities.guestcost
END FROM Facilities WHERE Bookings.facid = Facilities.facid ) AS cost
FROM Bookings
...
/* See 'SQLiteInstance.ipynb' for Q10 - Q13 */
